...
% table_engineqty_AR = AR;
% table_engineqty_EngineQty = type_engineqty;
% table_engineqty_DOC = DOC_perPAXMile;
% table_engineqty_Weight = W_takeoff;
...
%% Configuration Loop: Engine
i = 1;
for type_engineqty = 2:1:4
	j = 1;
	for AR = range_AR
		Weightincrement = 0;
	...
	%% Data Collection
		% Plots
		table_engineqty_AR(i,j) = AR;
		table_engineqty_EngineQty(i,j) = type_engineqty;
		table_engineqty_DOC(i,j) = DOC_perPAXMile;
		table_engineqty_Weight(i,j) = W_takeoff;
		table_engineqty_EngineMaterialCost(i,j) = C_TM_d_EngineMaterial;
		table_engineqty_EngineLabor(i,j) = C_TM_c_EngineLabor;
...
figure
hold on
for i = [1:1:(type_engineqty-1)]
	plot(table_engineqty_AR(i,:),table_engineqty_DOC(i,:))
end
legend('2','3','4')
title(legend,'Engine Quantity')
title('DOC wrt Engine Quantity')
xlabel('Aspect Ratio')
ylabel('DOC ($)')
hold off
...
hold on
for i = [1:1:(type_engineqty-1)]
	plot(table_engineqty_AR(i,:),table_engineqty_EngineMaterialCost(i,:))
end
legend('2','3','4')
title(legend,'Engine Quantity')
title('Engine Material Costs wrt Engine Quantity')
xlabel('Aspect Ratio')
ylabel('Material Costs ($)')
hold off

figure
hold on
for i = [1:1:(type_engineqty-1)]
	plot(table_engineqty_AR(i,:),table_engineqty_EngineLabor(i,:))
end
legend('2','3','4')
title(legend,'Engine Quantity')
title('Engine Labor Costs wrt Engine Quantity')
xlabel('Aspect Ratio')
ylabel('Labor Costs ($)')
hold off

%% Configuration Loop: Airfoil Shape
iteration = 1;
type_engineqty = 2;
i = 1;
for type_airfoil = 1:2
	if type_airfoil == 1
	% Figure 1a: Effect of Wing Sweep and Thickness on Mdiv (Conventional)
		x1 = Table(1:165,1);
		y1 = Table(1:165,3);
	...
	%% Data Collection
		% Plots
		table_airfoil_AR(i,j) = AR;
		table_airfoil_AirfoilType(i,j) = type_airfoil;
		table_airfoil_DOC(i,j) = DOC_perPAXMile;
		table_airfoil_Weight(i,j) = W_takeoff;
...
figure
hold on
for i = [1:1:type_airfoil]
	plot(table_airfoil_AR(i,:),table_airfoil_DOC(i,:))
end
legend('Conventional','Supercritical')
title(legend,'Airfoil Type')
title('DOC wrt Airfoil Type')
xlabel('Aspect Ratio')
ylabel('DOC ($)')
hold off