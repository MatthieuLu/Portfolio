...
l = 5; % number of lines in graphs at the end
...
iteration = 1;
table3d_AR = zeros(1,(length(range_AR)*length(range_Sweep)));
table3d_Sweep = zeros(1,(length(range_AR)*length(range_Sweep)));
table3d_DOC = zeros(1,(length(range_AR)*length(range_Sweep)));
table3d_Weight = zeros(1,(length(range_AR)*length(range_Sweep)));
table_CruiseSFC = zeros(1,(length(range_AR)*length(range_Sweep)));
table_Vcruise = zeros(1,(length(range_AR)*length(range_Sweep)));
table_LDcruise = zeros(1,(length(range_AR)*length(range_Sweep)));
table_FF = zeros(1,(length(range_AR)*length(range_Sweep)));
table_Wfuel = zeros(1,(length(range_AR)*length(range_Sweep)));
table_Payload = zeros(1,(length(range_AR)*length(range_Sweep)));
table_Thrust = zeros(1,(length(range_AR)*length(range_Sweep)));
table_S = zeros(1,(length(range_AR)*length(range_Sweep)));

table_AR = zeros(length(range_Sweep),length(range_AR));
table_Sweep = zeros(length(range_Sweep),length(range_AR));
table_DOC = zeros(length(range_Sweep),length(range_AR));
table_Weight = zeros(length(range_Sweep),length(range_AR));
...
%% Data Collection
		% Plots
		table3d_AR(iteration) = AR;
		table3d_Sweep(iteration) = Sweep;
		table3d_DOC(iteration) = DOC_perPAXMile;
		table3d_Weight(iteration) = W_takeoff;
		table_TC(iteration) = TC;
		table_AR(i,j) = AR;
		table_Sweep(i,j) = Sweep;
		table_DOC(i,j) = DOC_perPAXMile;
		table_Weight(i,j) = W_takeoff;
		table_b(i,j) = b;
		
		% Range Chart
		table_CruiseSFC(iteration) = SFC_cr;
		table_Vcruise(iteration) = V_cruise;
		table_LDcruise(iteration) = LD;
		table_FF(iteration) = FF;
		table_Wfuel(iteration) = W_fuel;
		table_Payload(iteration) = W_pl;
		table_Thrust(iteration) = Thrust_engine;
		table_S(iteration) = S;
...
[fit_DOC,info_DOC] = fit([table3d_AR',table3d_Sweep'],table3d_DOC','loess');
[fit_Weight,info_Weight] = fit([table3d_AR',table3d_Sweep'],table3d_Weight','loess');
[Min_DOC,optimallocation_DOC] = min(table3d_DOC);
[Min_Weight,optimallocation_Weight] = min(table3d_Weight);
optimal_DOC = table3d_DOC(optimallocation_DOC)
optimal_AR = table3d_AR(optimallocation_DOC)
optimal_Sweep = table3d_Sweep(optimallocation_DOC)
optimal_Weight = table3d_Weight(optimallocation_DOC)
optimal_Thrust = table_Thrust(optimallocation_DOC)
optimal_FF = table_FF(optimallocation_DOC)
optimal_b = table_b(optimallocation_DOC)
optimal_S = table_S(optimallocation_DOC)
optimal_TC = table_TC(optimallocation_DOC)
optimal_Weight_Weight = table3d_Weight(optimallocation_Weight);
optimal_Weight_AR = table3d_AR(optimallocation_Weight);
optimal_Weight_Sweep = table3d_Sweep(optimallocation_Weight);
		
%% Range Chart

% Values
max_Weight = table3d_Weight(optimallocation_DOC);
optimal_CruiseSFC = table_CruiseSFC(optimallocation_DOC);
optimal_Vcruise = table_Vcruise(optimallocation_DOC);
optimal_LDcruise = table_LDcruise(optimallocation_DOC);
optimal_FF = table_FF(optimallocation_DOC);
optimal_Wfuel = table_Wfuel(optimallocation_DOC);
optimal_Payload = table_Payload(optimallocation_DOC);
max_Payload = 52000;
delta_Payload = max_Payload - optimal_Payload;
% Point 1
p1_Weight0 = max_Weight;
p1_Payload = max_Payload;
p1_Range = 0;
% Point 2
p2_Weight0 = max_Weight;
p2_Payload = max_Payload;
p2_Wfuel = optimal_Wfuel - delta_Payload;
p2_Weight1 = p2_Weight0 - p2_Wfuel;
p2_Range = (optimal_Vcruise/optimal_CruiseSFC)*optimal_LDcruise*log(p2_Weight0/p2_Weight1);
% Point 3
p3_Weight0 = max_Weight;
p3_Payload = optimal_Payload;
p3_Wfuel = optimal_Wfuel;
p3_Weight1 = p3_Weight0 - p3_Wfuel;
p3_Range = (optimal_Vcruise/optimal_CruiseSFC)*optimal_LDcruise*log(p3_Weight0/p3_Weight1);
% Point 4
p4_Weight0 = max_Weight - optimal_Payload;
p4_Wfuel = optimal_Wfuel;
p4_Payload = 0;
p4_Weight1 = p4_Weight0 - p4_Wfuel;
p4_Range = (optimal_Vcruise/optimal_CruiseSFC)*optimal_LDcruise*log(p4_Weight0/p4_Weight1);
chart_Payload = [p1_Payload p2_Payload p3_Payload p4_Payload];
chart_Range = [p1_Range p2_Range p3_Range p4_Range];

%% Plots

% 3D DOC Plot
figure
scatter3(table3d_AR,table3d_Sweep,table3d_DOC);
hold on
plot(fit_DOC)
plot3(optimal_AR,optimal_Sweep,optimal_DOC,'r.','MarkerSize',30,'DisplayName','Point of Lowest DOC Possible')
title('DOC vs AR and Sweep')
xlabel('Aspect Ratio')
ylabel('Sweep Angle')
zlabel('DOC ($)')
hold off
% Contour DOC Plot
figure
[mesh_AR,mesh_Sweep] = meshgrid(range_AR,range_Sweep);
mesh_DOC = griddata(table3d_AR,table3d_Sweep,table3d_DOC,mesh_AR,mesh_Sweep);
[C,h] = contourf(mesh_AR,mesh_Sweep,mesh_DOC,20);
clabel(C,h)
hold on
plot(optimal_AR,optimal_Sweep,'r.','MarkerSize',30,'DisplayName',sprintf('Point of Lowest DOC Possible: %0.4f$',optimal_DOC))
plot(optimal_Weight_AR,optimal_Weight_Sweep,'y.','MarkerSize',30,'DisplayName',sprintf('Point of Lowest Weight Possible: %0.4flbs',optimal_Weight_Weight))
legend
title('DOC vs AR and Sweep: Composite')
xlabel('Aspect Ratio')
ylabel('Sweep Angle')
hold off
% 3D Weight Plot
figure
scatter3(table3d_AR,table3d_Sweep,table3d_Weight);
hold on
plot(fit_Weight)
plot3(optimal_AR,optimal_Sweep,optimal_Weight,'r.','MarkerSize',30,'DisplayName','Point of Lowest DOC Possible')
title('Take-off Weight vs AR and Sweep')
xlabel('Aspect Ratio')
ylabel('Sweep Angle')
zlabel('Weight (lbs)')
hold off
% Contour Weight Plot
figure
mesh_Weight = griddata(table3d_AR,table3d_Sweep,table3d_Weight,mesh_AR,mesh_Sweep);
[C,h] = contourf(mesh_AR,mesh_Sweep,mesh_Weight,20);
clabel(C,h)
hold on
plot(optimal_AR,optimal_Sweep,'r.','MarkerSize',30,'DisplayName',sprintf('Point of Lowest DOC Possible: %0.1flbs',optimal_Weight))
plot(optimal_Weight_AR,optimal_Weight_Sweep,'y.','MarkerSize',30,'DisplayName',sprintf('Point of Lowest Weight Possible: %0.1flbs',optimal_Weight_Weight))
legend
title('Take-off Weight vs AR and Sweep')
xlabel('Aspect Ratio')
ylabel('Sweep Angle')
hold off
% DOC wrt AR Plot
v = length(range_Sweep)/l;
figure
hold on
for i = 1:l
	plot(table_AR(round(i*v),:),table_DOC(round(i*v),:),'DisplayName',sprintf('%0.1f',range_Sweep(round(i*v))))
end
plot(optimal_AR,optimal_DOC,'r.','MarkerSize',30,'DisplayName','Lowest DOC Possible')
legend
title(legend,'Sweep Angle')
title('DOC wrt Aspect Ratio: Composite')
xlabel('Aspect Ratio')
ylabel('DOC ($)')
hold off
% DOC wrt Sweep Plot
...
hold on
for i = 1:l
	plot(table_Sweep(:,round(i*v)),table_DOC(:,round(i*v)),'DisplayName',sprintf('%0.1f',range_AR(round(i*v))))
end
plot(optimal_Sweep,optimal_DOC,'r.','MarkerSize',30,'DisplayName','Lowest DOC Possible')
legend
title(legend,'Aspect Ratio')
title('DOC wrt Sweep Angle: Composite')
xlabel('Sweep Angle')
ylabel('DOC ($)')
hold off
% Weight wrt AR Plot
...
hold on
for i = 1:l
	plot(table_AR(round(i*v),:),table_Weight(round(i*v),:),'DisplayName',sprintf('%0.1f',range_Sweep(round(i*v))))
end
plot(optimal_AR,optimal_Weight,'r.','MarkerSize',30,'DisplayName','Lowest DOC Possible')
legend
title(legend,'Sweep Angle')
title('Weight wrt Aspect Ratio')
xlabel('Aspect Ratio')
ylabel('Weight (lbs)')
hold off
% Weight wrt Sweep Plot
v = length(range_AR)/l;
figure
hold on
for i = 1:l
	plot(table_Sweep(:,round(i*v)),table_Weight(:,round(i*v)),'DisplayName',sprintf('%0.1f',range_AR(round(i*v))))
end
plot(optimal_Sweep,optimal_Weight,'r.','MarkerSize',30,'DisplayName','Lowest DOC Possible')
legend
title(legend,'Aspect Ratio')
title('Weight wrt Sweep Angle')
xlabel('Sweep Angle')
ylabel('Weight (lbs)')
hold off
% Range Chart
figure
plot(chart_Range,chart_Payload);
hold on
title('Effect of Payload Weight on Range')
xlabel('Range (mi)')
ylabel('Payload (lbs)')
hold off