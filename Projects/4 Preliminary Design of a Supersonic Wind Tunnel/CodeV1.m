...
efficiency = (((P02/P01)^((gamma-1)/gamma))-1)/((T02/T01)-1)
...
for n=1:length(x);
   if x(n) < inlength;
       M(n)=(M3-M2)/2*cos(1/3*pi*x(n)+pi)+(M3-M2)/2+M2;
   elseif x(n) >=inlength & x(n)<=(inlength+1);
       M(n)=M3;
%    elseif x(i) ==(inlength+1);
%        M(i)=M3s;
   elseif x(n) >(inlength+1) & x(n)<=(inlength+1+outlength-1);
       M(n)=(Ms2-M4)/2*cos(1/4*pi*x(n)-pi)-(Ms2-M4)/2+Ms2;
   else
       M(n)=M4;
   end   
end
